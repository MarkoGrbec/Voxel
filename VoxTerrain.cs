...
namespace VoxelTerrain
{
	...
	public class VoxTerrain : MonoBehaviour
	{
		...
		// 3d arrays
		public byte[,,] _map;
		public byte[,,] _clientMap;//{ get => _map; set {} }
		public short[,,] _colors;  // contains texture indexes, RGBA colors.
		public short[,,] _clientColors; //contains client indexes that were modified by server
		[HideInInspector]
		...
		public void Start()
		{
			...
			_clientColors ??= new short[width + 1, height + 1, depth + 1];
			_clientMap ??= new byte[width + 1, height + 1, depth + 1];
		...
		public void GetDataForClient(out short[] colors1d, out byte[] map1d,
			out int paramWidth, out int paramHeight, out int paramDepth)
		{
			colors1d = SaveLoad.colorsTo1D(_colors, width, height, depth);
			map1d = SaveLoad.mapTo1D(_map, width, height, depth);
			paramWidth = width;
			paramHeight = height;
			paramDepth = depth;
		}
		...
		public void CreateClientMap(short[] colors1d, byte[] map1d, int paramWidth, int paramHeight, int paramDepth)
		{
			width = paramWidth;
			height = paramHeight;
			depth = paramDepth;
			Start();
			_clientColors = SaveLoad.colorsTo3D(colors1d, width, height, depth);
			_clientMap = SaveLoad.mapTo3D(map1d, width, height, depth);
			// if (string.IsNullOrEmpty(TerrainName))
			// {
			// 	Debug.Log("TerrainName is null. Variable TerrainName need to be set.");
			// 	return;
		...
		//scalping modifying,call Draw3D to draw/sculpt things
		public void Draw3D(Vector3 position, Vector3 scale, OBJ obj, EFFECT BrushEffect, short textureID, bool doSculpt, bool doPaint, bool force, short[] material, bool drop, bool client = false)
		{
			...
			{
				InverseSphereEffectForSubtraction(bX, eX, bY, eY, bZ, eZ, center, sculptRadius, paintRadius, force, material, !drop, client);
			}
			// Begin of effect
			...
			switch (BrushEffect)
			{
				...
				case EFFECT.SUB:
					switch (obj)
					{
						...
						case OBJ.SPHERE: AddSphere(bounds, bX, bY, bZ, eX, eY, eZ, textureID, doSculpt, doPaint, BrushEffect, force, drop, client); break;
		...
		public bool SphereHasRock(Vector3 position, Vector3 scale, bool client = false)
		{
			// Get point in terrain location
			Matrix4x4 matrix = transform.worldToLocalMatrix;
			position = matrix.MultiplyPoint(position);
			...
			var sculptRadius = Mathf.Min(Mathf.Min(bounds.size.x, bounds.size.y), bounds.size.z) / 2F;
			var paintRadius = sculptRadius + 0.5f;
			for (var x = bX; x < eX; x++)
			{
				for (var y = bY; y < eY; y++)
				{
					for (var z = bZ; z < eZ; z++)
					{
						var distToCenter = Vector3.Distance(new Vector3(x, y, z), center);
						var dist = (distToCenter - sculptRadius) * 255;
		...
		// inverse effect for subtraction
		private void InverseSphereEffectForSubtraction(int bX, int eX, int bY, int eY, int bZ, int eZ, Vector3 center, float sculptRadius, float paintRadius, bool force, short[] material, bool dig, bool client)
		{
			for (var x = bX; x < eX; x++)
			{
				for (var y = bY; y < eY; y++)
				{
					for (var z = bZ; z < eZ; z++)
					{
						...
						if (client)
						{
							...
							// 	if (_clientColors[x, y, z] == 1)
							// 		break;
							// Debug.LogWarning($"the paint: {_clientColors[x, y, z]}");
							_clientMap[x, y, z] = (byte)(255 - _clientMap[x, y, z]);
						}
						else
						{
							// if (dist < 255 && !force)
						...
						if (dig)
						{
							//only server gets the digging number
							if (!client)
							{
								if (dist < 255 && material != null)
								{
									var index = _colors[x, y, z];
									if (index < 4)
									{
										material[index]++;
									}
									...
									{
										Debug.LogWarning($"white or empty??? {index}");
									}
								}
							}
						}
		...
		public bool LoadMapFromFile()
		{
			...
			{
				Instance.loadMap(TerrainName);//try{} 
														 //catch{UnityEngine.Debug.LogWarning ("Error while loading Terrain at path : (" + path + "). Loading empty world");}
				return true;
			}
			else
			{
				if (!firstLoad) Debug.LogWarning("Terrain at path : (" + path + ") not exist. Loading empty world");
				return false;
			}
		}

		public void DrawLoadedMap(bool client)
		{
		...
		/// 
		/// </summary>
		/// <param name="effect">for sub if it stops at rock</param>
		/// <param name="force">if it has pickaxe it can go through the rock</param>
		private void AddSphere(Bounds bounds, int bX, int bY, int bZ, int eX, int eY, int eZ, short textureID, bool doSculpt, bool doPaint, EFFECT effect, bool force, bool drop, bool client = false)
		{
			Vector3 center = bounds.center;
			float sculptRadius = Mathf.Min(Mathf.Min(bounds.size.x, bounds.size.y), bounds.size.z) / 2F;
			float paintRadius = sculptRadius + 0.5f;
				...
				for (int x = bX; x < eX; x++)
				{
					for (int y = bY; y < eY; y++)
					{
						for (int z = bZ; z < eZ; z++)
						{
							...
							if (dist < 255)
							{
								if (effect == EFFECT.ADD && !force)
								{
									if (!drop)
									{
										if (client)
										{
											_clientColors[x, y, z] = textureID;
										}
										else
										{
											_colors[x, y, z] = textureID;
							...
							byte xyz = _map[x, y, z];
							if (client)
								xyz = _clientMap[x, y, z];

							if (bVal < xyz)
							{
								if (client)
									_clientMap[x, y, z] = bVal;
								else
									_map[x, y, z] = bVal;
							}
						}
					...
					for (int y = bY; y < eY; y++)
					{
						for (int z = bZ; z < eZ; z++)
						{
							...
							if (dist < 255)
							{
								if (client)
								{
									if(! (_clientColors[x, y, z] == 1 && !drop))
										_clientColors[x, y, z] = textureID;
								}
								else
								{
									if(! (_colors[x, y, z] == 1 && !drop))
										_colors[x, y, z] = textureID;
								}
							}
						}
		...
		public void DrawPlaneTerrain()
		{
			...
			Draw3D(new Vector3(width / 2, 1, depth / 2), new Vector3(width, 1, depth), OBJ.PLANE, EFFECT.ADD, 0, true, true, true, null, false);
		...
		public void DrawTerrainFromHeightmap()
		{
			...
			Draw3D(new Vector3(width / 2, 0, height / 2), new Vector3(width, depth, height), OBJ.HEIGHTMAP, EFFECT.ADD, 0, true, true, true, null, false);
			DrawPlaneTerrain();
			// move camera right to get space from all sides (first voxel is on (0,0,0) pisition
			Camera.main.transform.position = new Vector3(width / 2, 25, -25);
			Camera.main.transform.RotateAround(new Vector3(125, 0, 20), Vector3.right, 45f);
