...
namespace VoxelTerrain
{
	public class Cube
	{
		...
		public Cube(bool client, Bounds bounds, VoxTerrain terrain)
		{
			...
			// Limit to all zone
			_terrain = terrain;
			_object = terrain.AddObject(client);
			_object.GetComponent<MeshFilter>().sharedMesh=new Mesh();
			_mesh = _object.GetComponent<MeshFilter>().sharedMesh; 
			_mesh.bounds = _bounds;
			if(client){
				_object.transform.position -= DeadByDawn.singleton.serverOffset;
			}
			_object.layer = 7;
			//visuals
		...
		public void ReBuild(bool client)
		{
			...
			if (client)
			{
				MarchingCubes.Polygonize(_terrain._clientMap,
					_minX, _minY, _minZ,
					_maxX, _maxY, _maxZ,
					out vertices,
					out triangles);
			...
			for(int i=0; i<vertices.Length; i++)
			{
				...
				// Index of texture
				if(client)
					colors[i] =  VoxTerrain.Instance.textureChannels[_terrain._clientColors[x, y, z]];
				else
